...
fn doc(manifest_path: Option<&Path>) -> anyhow::Result<()> {
    ...
    for (package, target) in library_crates.values() {
        let markdown = format!(
            "---\n\
             title: \"{} (<code>{}</code>)\"\n\
             documentation_of: //{}\n\
    ...
    impl<'cm> Tree<'cm> {
        fn insert(
            ...
            match (self, path) {
                (this, []) => {
                    *this = Self::Leaf(&package.id, target);
                }
...
fn library_crates(metadata: &cm::Metadata) -> BTreeMap<Vec<&str>, (&cm::Package, &cm::Target)> {
    ...
    metadata
        ...
        .flat_map(|ws_member| {
            ...
            let segments = ws_member.manifest_path.strip_prefix(crates_dir).ok()?;
            let segments = segments
                .iter()
                .take(segments.iter().count() - 1)
                .map(|segment| segment.to_str().expect("this is from a JSON"))
                .collect();
            Some((segments, (ws_member, target)))
        })
        .collect()
}
